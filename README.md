# ARISEN WASM Spec Tests

This repo provides a set of ARISEN unit tests that can be used to check a WASM Backend's conformance to the
WebAssembly spec.
...
6. The newly created merged wasms and unit test C++ files are copied into the appropriate directory in the rsn repo.
...
- Within a test suite, `assert_return` tests are grouped into sets of 100.
    - This is due to the limit on 1024 locals and 1024 func defs built into nodrsn. Some spec tests had too many functions to have a `sub_apply` per test, and some had too many variables to be put all into one `sub_apply`.
...
- Run the `setup_arisen_tests.py` script with no options to see the help text.
...
- start.7 -- Will fail if not deleted from generated tests.
    - Imports "print" from "spectest". Changing to any of the ARISEN print functions results in "start function must be nullary" due to their requiring a parameter.

- globals.2 -- Delete from generated tests or it segfaults due to missing wasm.
    - `arisen-wasm2wast` error "mutable globals cannot be exported" when converting to wast.
...
- globals.14 -- Delete from generated tests or it segfaults due to missing wasm.
    - Imports "global_i32" from "spectest".
        - Unclear what an appropriate substition from ARISEN would be.
